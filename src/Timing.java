...
public class Timing {
	...
	public static long linearithmic(int paramInt1, int paramInt2) {
		...
		for (int i = 0; i < paramInt1; ++i)
			l += constant(paramInt2);
		return (linearithmic(paramInt1 / 2, paramInt2) + l + linearithmic(
	...
	private static long exponential(int paramInt1, int paramInt2) {
		if (paramInt1 == 0)
			return constant(paramInt2);
		return (exponential(paramInt1 - 1, paramInt2) + exponential(
	...
	private static long divideAndConquer(int paramInt1, int paramInt2,
			int paramInt3, int paramInt4) {
		if (paramInt1 == 0)
			return 0L;
		long l = constant(paramInt2);
		for (int i = 0; i < paramInt3; ++i) {
			l += divideAndConquer(paramInt1 / paramInt4, paramInt2, paramInt3,
					paramInt4);
		}
		return l;
	}

	private static long powerLaw(int paramInt1, int paramInt2,
			double paramDouble) {
		long l1 = 0L;
